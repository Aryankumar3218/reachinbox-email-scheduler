...
import { X, Upload, CheckCircle, FileText, Clock, ShieldAlert, Send, Mail } from 'lucide-react';
      ...
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Schedule Email Campaign</h2>
              <p className="text-xs text-slate-500">Configure outbound queue, schedule window, and rate limits</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-slate-700">Sender Account</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <select
                value={sender}
                onChange={(e) => setSender(e.target.value)}
                className="bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              >
          ...
          <div className="space-y-1.5">
            ...
            <div
              ...
              className="border-2 border-dashed border-slate-300 hover:border-blue-400 bg-slate-50 hover:bg-slate-100/60 rounded-xl p-4 text-center cursor-pointer transition"
            >
              <input
                ref={fileInputRef}
                type="file"
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">
                Supports standard comma-delimited columns (e.g. sample-leads.csv, indian-leads.csv)
              </p>
            </div>
            ...
            <div className="mt-2">
              <textarea
                rows={2}
                placeholder="Or paste comma-separated emails: aarav.sharma@tcs.com, priya.sharma@apexreach.co"
                value={manualRecipientsInput}
                onChange={(e) => handleManualRecipientsChange(e.target.value)}
                className="w-full bg-white border border-slate-300 rounded-lg p-2.5 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 font-mono"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              placeholder="e.g. ReachInbox partnership opportunity"
              value={subject}
              onChange={(e) => setSubject(e.target.value)}
              className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              required
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              rows={4}
              placeholder="Hi there,&#10;&#10;I wanted to connect to share how we can accelerate your outreach workflows. Let me know if you are open to a brief discussion this week.&#10;&#10;Best regards,"
              value={body}
              onChange={(e) => setBody(e.target.value)}
              className="w-full bg-white border border-slate-300 rounded-lg p-3 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 leading-relaxed"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-200">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-200">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition disabled:opacity-50 cursor-pointer"
            >
